...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar.find(item => item.id == aktifID).isim;
  }

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: aktifID != null ? 'gold': 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setAktifID(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
